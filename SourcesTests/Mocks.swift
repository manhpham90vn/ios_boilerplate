...
class AppApiMock: AppApi {
    ...
    private(set) var pagingCallCount = 0
    var pagingArgValues = [(Int, PagingSortType)]()
    var pagingHandler: ((Int, PagingSortType) -> (Single<[PagingUserResponse]>))?
    func paging(page: Int, sort: PagingSortType) -> Single<[PagingUserResponse]> {
        pagingCallCount += 1
        pagingArgValues.append((page, sort))
        if let pagingHandler = pagingHandler {
            return pagingHandler(page, sort)
        }
        fatalError("pagingHandler returns can't have a default value thus its handler must be set")
    }
...
class StorageMock: Storage {
    ...
    var setStringArgValues = [(String, String)]()
    var setStringHandler: ((String, String) -> ())?
    func setString(key: String, value: String)  {
        setStringCallCount += 1
        setStringArgValues.append((key, value))
        if let setStringHandler = setStringHandler {
            setStringHandler(key, value)
        }
        
    }

    private(set) var getStringCallCount = 0
    var getStringArgValues = [String]()
    var getStringHandler: ((String) -> (String?))?
    func getString(key: String) -> String? {
        getStringCallCount += 1
        getStringArgValues.append(key)
        if let getStringHandler = getStringHandler {
            return getStringHandler(key)
        }
        return nil
    }

    private(set) var setObjectCallCount = 0
    var setObjectArgValues = [(String, Any)]()
    var setObjectHandler: ((String, Any) throws -> ())?
    func setObject<T: Codable>(key: String, value: T) throws  {
        setObjectCallCount += 1
        setObjectArgValues.append((key, value))
        if let setObjectHandler = setObjectHandler {
            try setObjectHandler(key, value)
        }
        
    }

    private(set) var getObjectCallCount = 0
    var getObjectArgValues = [String]()
    var getObjectHandler: ((String) throws -> (Any))?
    func getObject<T: Codable>(key: String) throws -> T {
        getObjectCallCount += 1
        getObjectArgValues.append(key)
        if let getObjectHandler = getObjectHandler {
            return try getObjectHandler(key) as! T
        }
        fatalError("getObjectHandler returns can't have a default value thus its handler must be set")
    }

    private(set) var clearCallCount = 0
    var clearArgValues = [String]()
    var clearHandler: ((String) -> ())?
    func clear(key: String)  {
        clearCallCount += 1
        clearArgValues.append(key)
